...
ALTER TABLE `cliente_gp_areas` DROP INDEX IF EXISTS `IDX_areas_locationId`;

-- Step 2: Remove the foreign key constraint (if it exists)
-- Note: Check your database if foreign keys are enforced
-- ALTER TABLE `cliente_gp_areas` DROP FOREIGN KEY IF EXISTS `FK_areas_locationId`;

-- Step 3: Remove the locationId column
ALTER TABLE `cliente_gp_areas` DROP COLUMN IF EXISTS `locationId`;
